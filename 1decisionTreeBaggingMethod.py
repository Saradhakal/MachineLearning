...
rf_bagging_model = BaggingClassifier(base_estimator=DecisionTreeClassifier(),n_estimators=100, bootstrap=True, max_samples=100, n_jobs=-1, random_state=0)

rf_bagging_model.fit(x_train , y_train)
rf_bagging_train_score = rf_bagging_model.score(x_train , y_train)

print("DT Bagging Classifier Training Score: {:.3F}".format(rf_bagging_train_score))
